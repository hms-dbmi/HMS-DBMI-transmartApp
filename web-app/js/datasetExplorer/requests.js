...
function getCRCQueryRequest(subset, queryname)
{
	...
	for(var i=1;i<=GLOBAL.NumOfQueryCriteriaGroups;i++)
	{
		...
		{
			query=query+getCRCRequestPanel(qcd.dom, i, queryTiming);
		}
	}

	var queryHead = getCRCRequestHeader()+ '<user group="'+GLOBAL.ProjectID+'" login="'+GLOBAL.Username+'">'+GLOBAL.Username+'</user>\
	...
	query = queryHead + query + getSecurityPanel() + "</query_definition>" + getCRCRequestFooter();

	return query;
}

//takes actual dom element
function getCRCRequestPanel(qd, number, queryTiming) 
{
	//set the invert
	var invert=0;
	
	if(jQuery("#" + qd.id).hasClass("queryGroupExclude")){
		invert=1;
	}
	
	//set the Ngs
	var panelLinking="ANY";
	
	if(jQuery("#" + qd.id).hasClass("queryGroupSameTiming")){
		panelLinking="SAMEVISIT";
		queryTiming="SAMEVISIT";
	}
	...
	var panel="<panel>\
	               <panel_number>"+number+"</panel_number>\
	                    ...
	                    <panel_timing>"+panelLinking+"</panel_timing>\
...
function getCRCRequestItem(el){
    ...
    if(el.getAttribute("concepttablename") == "MODIFIER_DIMENSION")
    {
   
    	var hlevel 				= el.getAttribute("modifiednodelevel");
    	var item_name			= el.getAttribute("modifiednodeid").replace("<", "&lt;").replace(">", "&gt;");
    	var modified_node_path	= el.getAttribute("modifiednodepath");
    	var modified_node_id	= el.getAttribute("modifiednodeid").replace("<", "&lt;").replace(">", "&gt;");	
    	var applied_path		= el.getAttribute("applied_path");
    	var modifier_key		= el.getAttribute("conceptid");
    	
    	//We'll hijack modifier name if we are constraining by a value.
    	var modifier_name		= el.getAttribute("conceptname");
    	
  	  	if(el.getAttribute("setvaluemode")=='Enum')
  	  	{
  	  		modifier_name +=  " in (" + el.getAttribute("conceptsetvaluetext") + ") ";
  	  	}
    	...
    	var item=		'<item>\
	                        ...
	                        <item_key>' + modified_node_path + '</item_key>\
	                        <tooltip>' + modified_node_id + '</tooltip>\
	                        <class>ENC</class>';
    	
  	  	item=item+'<constrain_by_modifier>';
  	  	
  	  	item=item+'<modifier_name>' + modifier_name + '</modifier_name>';
  	  	item=item+'<applied_path>' + applied_path + '</applied_path>';
  	  	item=item+'<modifier_key>' + modifier_key + '</modifier_key>';
  	  	
  	  	if(el.getAttribute("setvaluemode")=='Enum')
  	  	{
  	  		item=item+'<constrain_by_value>';
  	  		item=item+'	<value_type>TEXT</value_type>';
  	  		item=item+"	<value_constraint>('" + el.getAttribute("conceptsetvalueinlist").split(",").join("','") + "')</value_constraint>";
  	  		item=item+"	<value_text>" + el.getAttribute("conceptsetvaluetext") + "</value_text>";
  	  		item=item+'	<value_operator>IN</value_operator>';
  	  		item=item+'</constrain_by_value>';
  	  	}
    ...
    else
    {
		...
		//Construct part of the XML document being sent to i2b2 services. Escape when there are < or > in the concept path.
	 	var item=		'<item>\
	                        ...
	                        <item_name>'+el.getAttribute("conceptid").replace("<", "&lt;").replace(">", "&gt;")+'</item_name>\
		...
		var root = climbTreeBuildName(el);
		
		// Ephi 12/3/14: By default constrain by PATIENT_VALID modifier
		// Note that this needs to be implemented also for when actually constraining by modifiers, since PATIENT_VALID is set 
		// to 0 for negative polarity etc. and also for user specificed invalidation.
		// But when the user manually searches for negative polarity, PATIENT_VALID should only take into account the user specified
		// invalidation.	
		// TODO: This needs a better configuration soluation.
		if(el.getAttribute("conceptid").indexOf(GLOBAL.conceptPathForValidation) > -1)
		{
			item=item+'<constrain_by_modifier>';
			item=item+'<applied_path>' + GLOBAL.conceptPathForValidation + '%' + '</applied_path>';
			item=item+'<modifier_key>' + GLOBAL.modifierPathForValidation + '</modifier_key>';  	  	
			item=item+'<constrain_by_value>';
			item=item+'	<value_operator>IN</value_operator>';
			item=item+"	<value_constraint>('1')</value_constraint>";
			item=item+'	<value_type>TEXT</value_type>';
			item=item+'</constrain_by_value>';  	  	
	  	  	item=item+'</constrain_by_modifier>';
		}
		
       	if(el.getAttribute("conceptid").indexOf(GLOBAL.conceptPathForValidationPublic) > -1)
		{
			item=item+'<constrain_by_modifier>';
			item=item+'<applied_path>' + GLOBAL.conceptPathForValidationPublic + '%' + '</applied_path>';
			item=item+'<modifier_key>' + GLOBAL.modifierPathForValidationPublic + '</modifier_key>';  	  	
			item=item+'<constrain_by_value>';
			item=item+'	<value_operator>IN</value_operator>';
			item=item+"	<value_constraint>('1')</value_constraint>";
			item=item+'	<value_type>TEXT</value_type>';
	      ...
	      if(el.getAttribute("setvaluemode")=='numeric')
	      {
	            ...
	            {
	            	item=item+'<value_constraint>'+el.getAttribute("setvaluelowvalue")+'</value_constraint>'
	            }
