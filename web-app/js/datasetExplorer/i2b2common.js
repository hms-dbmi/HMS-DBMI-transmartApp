...
function convertNodeToConcept(node)
{
	...
	concept.name			=	node.text;
	concept.level			=	node.attributes.level;
	concept.key				=	node.id;
	concept.tooltip			=	node.attributes.qtip;
	concept.tablename		=	node.attributes.tablename;
	concept.dimcode			=	node.attributes.dimcode;
	concept.comment			=	node.attributes.comment;
	concept.normalunits		=	node.attributes.normalunits;
	concept.oktousevalues	=	node.attributes.oktousevalues;
	concept.applied_path	=	node.attributes.applied_path;
	concept.nodeType		=	node.attributes.iconCls;
	...
	concept.modifiedNode = modifiedNode;

	var value = new Value();
	
	if(concept.oktousevalues=="Y"){value.mode="numeric";} //default to numeric
...

{
	...
	var shortname = extractConceptShortName(concept.tooltip);
	...
	new Ext.ToolTip({ target:li, html:concept.tooltip, dismissDelay:10000 });
...
function extractConceptShortName(conceptPath)
{
...

{
	...
	Ext.get("btnEncounterGroup"+subset+"_"+panel).dom.firstChild.nodeValue="Enable Variant Panel"
	...
	//clear GLOBAL.concept_relation
	if (subset == 3)
	{
		GLOBAL.relation_concept = '';
	}
...

{
	...
	if(jQuery("#queryCriteriaDiv"+subset+"_"+panel).hasClass("queryGroupInclude")){
		
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).addClass("queryGroupExclude");
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).removeClass("queryGroupInclude");

		button.firstChild.nodeValue="Include";
	}else{
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).addClass("queryGroupInclude");
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).removeClass("queryGroupExclude");		
...
//This fires off when the panel timing is toggled with the 'Linked Panel' button.
function linkedPanelToggle(btn,subset, panel)
{
	var el 						= Ext.get("queryCriteriaDiv"+subset+"_"+panel);
	var button 					= Ext.get("btnEncounterGroup"+subset+"_"+panel).dom;
	
	var currentDivClassName		= el.dom.className
	
	if(jQuery("#queryCriteriaDiv"+subset+"_"+panel).hasClass("queryGroupSameTiming"))
	{
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).addClass("queryGroupAnyTiming");
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).removeClass("queryGroupSameTiming");
		
		button.firstChild.nodeValue="Enable Variant Panel";
	}
	else
	{
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).addClass("queryGroupSameTiming");
		jQuery("#queryCriteriaDiv"+subset+"_"+panel).removeClass("queryGroupAnyTiming");		

		button.firstChild.nodeValue="Disable Variant Panel";
	}
...
function getQuerySummaryItem(el){
	...
	if(el.getAttribute("applied_path") == '@')
	{
		conceptToolTip = el.getAttribute("concepttooltip")
	...
	{
		conceptToolTip = el.getAttribute("modifiednodeid")
		
		if(el.getAttribute("setvaluemode") == "Enum")
		{
			item = conceptToolTip + " " + el.getAttribute("conceptsetvaluetext");
		}
		else
		{
			item = conceptToolTip  + " [" + el.getAttribute("conceptshortname") + "]";
		}
	}
...

{
...

	{
		...
		if(queryDiv && queryDiv.dom.childNodes.length>0)
...
function getTreeNodeFromXMLNode(concept)
{
	    ...
	    if(tablename == "MODIFIER_DIMENSION")
	    {
	    	iconCls			= "modifiericon";
	    	applied_path 	= concept.selectSingleNode('applied_path').firstChild.nodeValue;
	    	modifierId 		= concept.selectSingleNode('key').firstChild.nodeValue;
	    }
    	...
    	{
	    	var char = visualattributes.substr(2,1).toLowerCase();
	    	iconCls=char+"leaficon";
    		tcls=char+"leafclass";
    		if (char == 'b')
    		{
    			tcls = tcls + " editable-blue";
    		}
    	}
	    ...
	    else if(nodetype=='C') //folder-not-dragable
	    {
	    	leaf=false;
	    	draggable=false;
	    }    
	    ...
	    else if(nodetype=='R') //Modifier
	    {
	    	leaf=true;
	    	draggable=true;   
	    }    
	    else if(nodetype=='O') //Modifier-Container
	    {
	    	leaf=false;
	    	draggable=false;
	    ...
	    else if(nodetype=='D') //Modifier-folder
	    {
	    	leaf=false;
	    	draggable=true;
