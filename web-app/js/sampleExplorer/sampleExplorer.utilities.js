...
function createValuesString(JSONObject)
{
    ...
    for(var key in JSONObject)
    {
    	...
    	if(JSONObject[key].length > 0 && (key != "GridColumnList") && (key != "result_instance_id")) 
		{
    		...
    		{
    			valueString += newKey + " " + JSONObject[key][0][1] + " " + JSONObject[key][0][0];   
    		}
    		else
    		{
...
function intializeIndividualSearchBoxes(forceGridUpdate)
{
	for(var loopCounter=0; loopCounter < GLOBAL.columnMap.columns.length; loopCounter++){

		//Only if this column is to appear in the grid do we add it here.
		if(GLOBAL.columnMap.columns[loopCounter].type == "string_autocomplete" && (forceGridUpdate || GLOBAL.columnMap.columns[loopCounter].mainTerm))
		{
			createSearchBoxIndividual("search-text-" + GLOBAL.columnMap.columns[loopCounter].dataIndex, GLOBAL.columnMap.columns[loopCounter].dataIndex,forceGridUpdate);
		}
	}
}
...
function getFieldListForColModel()
{
	...
	else if(GLOBAL.explorerType == "clinicalNoteExplorer")
	{
		returnColumnMaps.push({header:'Term Count',dataIndex:'count', width:'5'})	
	}
	else if(GLOBAL.explorerType == "sequenceVariantExplorer")
	{
		returnColumnMaps.push({header:'Variant Count',dataIndex:'count', width:'5'})	
	}
