...
function toggleGridDisplay()
{
	...
	Ext.getCmp('westPanel').load(
			{
				...
				callback : initializeWestSearchBoxes
			}
		  );
	...
	Ext.getCmp('savesearchbutton').setVisible(true);
	Ext.getCmp('saveSearchQueryName').setVisible(true);
...
function toggleMainCategoryFilterComplete(filterData)
{
	initializeGlobalGridColumnList();
	
	var JSONData = Ext.decode(filterData)
	
	GLOBAL.SearchJSON = JSONData
...
//When a main category is selected we need to reload both panels.
function toggleMainCategoryRange(searchValue,searchDirection,searchCategory)
{
	initializeGlobalGridColumnList();

	GLOBAL.SearchJSON[searchCategory] = [];
	
	//Push the value and search direction into the array for its category.
	GLOBAL.SearchJSON[searchCategory].push([searchValue, searchDirection]);
 ...
 */
function updateFilterList(newSearchTerm,checkedStatus,searchCategory, renderGrid)
{
	renderGrid = typeof renderGrid !== 'undefined' ? renderGrid : true;
...
function updateRangeList(searchValue,searchDirection,searchCategory,renderGrid)
{
	if(searchValue != '')
	{
		renderGrid = typeof renderGrid !== 'undefined' ? renderGrid : true;
	...
	{
		document.getElementById("checkbox_" + searchCategory).checked = false;
		alert('Please enter a value for ' + searchCategory);
	}
}

function initializeWestSearchBoxes()
{
	intializeIndividualSearchBoxes(true);
	populateIndividualSearchBoxes();
}

function populateIndividualSearchBoxes()
{
	for(var columnsLoopCounter=0; columnsLoopCounter < GLOBAL.columnMap.columns.length; columnsLoopCounter++){

		//Only if this column is to appear in the grid do we add it here.
		if(GLOBAL.columnMap.columns[columnsLoopCounter].type == "string_autocomplete")
		{
			var currentSelectedValuesArray = GLOBAL.SearchJSON[GLOBAL.columnMap.columns[columnsLoopCounter].dataIndex]
			
			if(currentSelectedValuesArray)
			{
				//For each autocomplete section, create a div for any values already selected.
				for(var valuesLoopCounter=0; valuesLoopCounter < currentSelectedValuesArray.length; valuesLoopCounter++)
				{
					addCategorySelectionDiv("search-text-" + GLOBAL.columnMap.columns[columnsLoopCounter].dataIndex, currentSelectedValuesArray[valuesLoopCounter], GLOBAL.columnMap.columns[columnsLoopCounter].dataIndex, true); 
				}
			}
		}
	}	
