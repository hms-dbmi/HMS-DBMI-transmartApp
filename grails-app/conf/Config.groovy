...
println("Attempting to include configuration files from ${userHome}/.grails/${appName}Config/")
...
println("Building Config Locations")
...
println("Get config location from environment variables.")
def externalConfig = System.getenv("${bashSafeEnvAppName}_CONFIG_LOCATION")
if (externalConfig) {
	grails.config.locations << "file:" + externalConfig
}

println("Get datasource config location from environment variables.")
...
grails.plugins.springsecurity.filterChain.filterNames = [
	...
	'auth0AuthenticationFilter', // was 'authenticationProcessingFilter',
	'logoutFilter',
