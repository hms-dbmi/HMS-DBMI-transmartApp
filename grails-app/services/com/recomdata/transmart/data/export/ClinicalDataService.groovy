...
class ClinicalDataService {
	...
	public boolean getData(List studyList, File studyDir, String fileName, String jobName, String resultInstanceId, 
		...
		if(null != resultInstanceId)
		{
			if(conceptCodeList.size() > 1000)
			{
				...
				sql.execute(" DELETE FROM I2B2DEMODATA.QT_CONCEPT_CD_COLLECTION where Result_Instance_Id ='${resultInstanceId}' ");
				
				int size_arr = 0;
				
				while(size_arr<conceptCodeList.size())
				{
							sql.execute("insert into I2B2DEMODATA.QT_CONCEPT_CD_COLLECTION(CONCEPT_CD,Result_Instance_Id) values (?,?)",[conceptCodeList[size_arr],resultInstanceId]);
							size_arr = size_arr +1;
		...
		if (null != resultInstanceId) {
			...
			if(conceptCodeList.size() > 1000)
			{
				...
				sqlQuery <<= " AND cd.CONCEPT_CD IN (select CONCEPT_CD from I2B2DEMODATA.QT_CONCEPT_CD_COLLECTION where Result_Instance_Id ='${resultInstanceId}') "
	...
	private void pivotData(boolean multipleStudies, String study, String inputFileLoc, boolean mRNAExists, boolean snpExists) {
		if (StringUtils.isNotEmpty(inputFileLoc)) {
			...
			if (null != inputFile) {
				...
				log.debug("Pivot working directory command : " + workingDirectoryCommand)
				...
				log.debug("Pivot compile command : " + compilePivotDataCommand)
				...
				log.debug("Pivot command : " + pivotDataCommand)
	...
	public retrieveClinicalData(String resultInstanceId, String[] conceptPaths, boolean gatherAllEncounterFacts = false)
	{
		
		def sqlParameterList = new ArrayList();
		
		sqlParameterList.add(resultInstanceId)

		def sqlQuery = new StringBuilder();
		groovy.sql.Sql sql = new groovy.sql.Sql(dataSource)
		
		sqlQuery <<= "SELECT ofa.ENCOUNTER_NUM, ofa.PATIENT_NUM, cd.CONCEPT_PATH, cd.CONCEPT_CD, cd.NAME_CHAR, "
		sqlQuery <<= "case ofa.VALTYPE_CD "
		sqlQuery <<= " WHEN 'T' THEN TVAL_CHAR "
		sqlQuery <<= " WHEN 'N' THEN CAST(NVAL_NUM AS varchar2(30)) "
		sqlQuery <<= "END VALUE, pd.sourcesystem_cd, pd.race_cd "
		sqlQuery <<= "FROM qt_patient_set_collection qt "
		sqlQuery <<= "INNER JOIN OBSERVATION_FACT ofa ON qt.PATIENT_NUM = ofa.PATIENT_NUM "
		sqlQuery <<= "INNER JOIN CONCEPT_DIMENSION cd ON cd.CONCEPT_CD = ofa.CONCEPT_CD "
		sqlQuery <<= "INNER JOIN PATIENT_DIMENSION pd on ofa.patient_num = pd.patient_num "
		sqlQuery <<= "WHERE qt.RESULT_INSTANCE_ID = ? AND ("
		
		def conceptCounter=0;
		
		conceptPaths.each{
			sqlParameterList.add(it)
			
			if(conceptCounter > 0) sqlQuery <<= " OR "
			
			sqlQuery <<= " CONCEPT_PATH=? "
		...
		def rowReturnObject = []
		def distinctEncounterNums = []
		
		try 
		{
			def rows = sql.rows(sqlQuery.toString(), sqlParameterList)
			...
			if (rows.size() > 0) {
				...
				rows.each { row ->
					...
					values["ENCOUNTER_NUM"]=row.ENCOUNTER_NUM?.toString()
					values["PATIENT_NUM"]=row.PATIENT_NUM?.toString()
					values["RACE_CD"]=row.RACE_CD?.toString()
					
					values["CONCEPT_PATH"]=row.CONCEPT_PATH?.toString()
					values["CONCEPT_CD"]=row.CONCEPT_CD?.toString()
					values["VALUE"]=row.VALUE?.toString()
					
					distinctEncounterNums.add(row.ENCOUNTER_NUM?.toString())
					
					rowReturnObject.add(values)
				}
	
			}
		} catch (Exception e) {
			log.info(e.getMessage())
			//TODO: Couldn't get this to throw properly.
			//throw new ClinicalDataFetchException("Failed to fetch data! - " + e.getMessage())
		} finally {
			sql?.close()
		}
		
		
		if(gatherAllEncounterFacts)
		{
			distinctEncounterNums.unique()
			def relatedFacts = gatherAllFactsFromEncounter(distinctEncounterNums)
			rowReturnObject.addAll(relatedFacts)
		}
		
		return rowReturnObject
	}
	
	private List<Object> gatherAllFactsFromEncounter(distinctEncounterNums)
	{
		//We need to find the other variant data.
		def sqlQuery = new StringBuilder();
		...
		sqlQuery <<= "INNER JOIN CONCEPT_DIMENSION cd ON cd.CONCEPT_CD = ofa.CONCEPT_CD "
		sqlQuery <<= "INNER JOIN PATIENT_DIMENSION pd on ofa.patient_num = pd.patient_num "
		sqlQuery <<= "WHERE ENCOUNTER_NUM IN ("
	
		def conceptCounter=0;

		distinctEncounterNums.each{
			sqlParameterList.add(it)
			
			if(conceptCounter > 0) sqlQuery <<= " , "
...
class ClinicalDataFetchException extends RuntimeException {
	String message
}
