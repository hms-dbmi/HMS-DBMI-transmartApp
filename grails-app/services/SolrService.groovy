...
class SolrService {
	 ...
	 * @return
	 */
	def pullResultsBasedOnJson(JSONObject,String resultColumns, Boolean enforceEmpty, Boolean addSort, coreName)
	{
		//Get the solr Query based on the JSON object.
		def solrQuery = ""
		...
		if(addSort) solrQuery += "&fl=" + resultColumns + "&sort=id desc"
		solrQuery += "&rows=" + solrMaxRows
	 ...
	 */
	def suggestTerms(String fieldList, String termPrefix,String numberOfSuggestions, String coreName)
	{
		...
		solrQuery['terms.regex'] = termPrefix.toString() + ".*"
		solrQuery['rows'] = numberOfSuggestions
		solrQuery['terms.regex.flag'] = 'case_insensitive'
		
		//fieldList.tokenize(",").each()
		//{
			solrQuery['terms.fl'] = fieldList.tokenize(",")
		//}
		
		//Facet the search on the field specified in the parameter.
		def html = http.get( path : '/solr/' + coreName +'/terms', query :  solrQuery)
		{
			resp, xml ->
		...
		JSONObject.each
		{
			...
			if(category.value.size() > 0 && category.key.toString() != "count" && !category.key.toString().startsWith("GridColumnList") && !category.key.toString().startsWith("result_instance_id"))
			{
				...
				category.value.each
				{
					...
					if(categoryItem.getClass() == org.codehaus.groovy.grails.web.json.JSONArray)
					{
						
						String categoryValue = categoryItem[0].toString()
						String categoryDirection = categoryItem[1].toString()
						
						if(categoryDirection == "<")
						{
							solrQuery += category.key.toString() + ":[ * TO " + categoryValue + "]"
						}
						else
						{
							solrQuery += category.key.toString() + ":[ " + categoryValue + " TO * ]"
						}
   ...
   def saveSolrQuery(queryName, queryContents)
   {
	   def newSavedQuery = new SaveSolrQuery(queryName: queryName, queryContents: queryContents)
	   newSavedQuery.save()
   }
   
   def deleteSolrQuery(queryIdToDelete)
   {
	   def savedQueryToDelete = SaveSolrQuery.get(queryIdToDelete)
	   savedQueryToDelete.delete()
   }
