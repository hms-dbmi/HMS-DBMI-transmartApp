...
<table <g:if test="${initialPage == true}">class="categoryList"</g:if><g:else>class="categoryListCheckbox"</g:else>>
	<TBODY>
	...
	<g:each var="term" in="${termList}" status="iterator" >
		...
		<tr>
			<td>
				<g:if test="${initialPage == true}">
					<input type = "checkBox" name="${termName}_${term.key}" onClick="updateFilterList('${term.key.replace("'","\\'")}',this.checked,'${termName}', false);" />
				</g:if>
				<g:elseif test="${JSONData[termName]?.contains(term.key)}">
					<input type = "checkBox" name="${termName}_${term.key}" checked onClick="updateFilterList('${term.key.replace("'","\\'")}',this.checked,'${termName}');" />
				</g:elseif> 
				<g:else>
					<input type = "checkBox" name="${termName}_${term.key}" onClick="updateFilterList('${term.key.replace("'","\\'")}',this.checked,'${termName}');" />
				</g:else> 
