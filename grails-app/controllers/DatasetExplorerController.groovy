...
class DatasetExplorerController {
	...
	def index = {
		...
		// If this is a GRDR instance, we do not need to restrict
		// non-admin users to 'SECURE' PublicStudies only (see extra
		// security panel addition in datasetExplorer.js and request.js)
		def isPublicStudySecured = Holders.config.edu.harvard.transmart.isPublicStudyRestricted.equalsIgnoreCase('true')
		...
		render (view:"datasetExplorer", model: [pathToExpand: pathToExpand,
			admin: admin,
			ispublicsecure: isPublicStudySecured,
			tokens: tokens,
