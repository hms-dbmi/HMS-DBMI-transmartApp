...
class ClinicalNoteExplorerController {
	...
	def index =
	{
		...
		def al = new AccessLog(username: springSecurityService.getPrincipal().username, event:"ClinicalNoteExplorer-Summary", eventmessage:"Clinical Note Explorer summary page", accesstime:new Date())
		al.save();
	...
	def showTopLevelListPage =
	{
		...
		def termMap = solrService.facetSearch("",verifyFieldList(), 'clinicalNoteExplorer')
	...
	def list =
	{
		...
		render(view: "clinicalNoteExplorer", model:[sampleRequestType:"search", columnData:columnMap as JSON])
	}
	...
	def showWestPanelSearch =
	{
		...
		def termMap = solrService.facetSearch(request.JSON.SearchJSON,solrFieldList, 'clinicalNoteExplorer')
	...
	def loadSearch =
	{
		...
		def resultsHash = solrService.suggestTerms(category,values,grailsApplication.config.com.recomdata.solr.numberOfSuggestions.toString(), 'clinicalNoteExplorer')
	...
	def loadCategories =
	{
		...
		categoryMap['rows'].add(['value':'STR','label':'Term'])
	...
	def loadFieldList = {
		...
		def resultsList = grailsApplication.config.clinicalNoteExplorer.fieldMapping.clone()
	...
	def loadEntireFieldList = {
		...
		//Pull the field map from the configuration file.
		def fullColumnList = grailsApplication.config.clinicalNoteExplorer.fieldMapping.clone()
	...
	def getDataSetResults =
	{
		...
		def resultsHash = solrService.pullResultsBasedOnJson(request.JSON.SearchJSON,selectedResultColumns, false, 'clinicalNoteExplorer')
