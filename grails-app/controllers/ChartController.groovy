...
class ChartController {
    ...
    def childConceptPatientCounts = {
			...
			def concept_level = params.concept_level;
			...
			def metadata	= nodeMetadataService.retrieveNodeStats(keyToPath(concept_key), columnMap)
			...
			//Performance reasons dictate a lookup by level when we have it.
			if(concept_level)
			{
				concept = Concept.findByLevelAndFullName(concept_level, keyToPath(concept_key));
			}
			else
			{
				concept = Concept.findByFullName(keyToPath(concept_key));
			}
			
			def validatedConcepts = nodeMetadataService.retrieveConceptsWithPositiveModifier(concept, conceptValidatedModifier);
			validatedConcepts.each { validatedConcept -> validated[validatedConcept] = 1 }
			
						
    		log.debug("access:"+(access as JSON));
    		log.debug("counts = " + (counts as JSON))

    		def obj=[counts:counts, accesslevels:access, metadata:metadata, validated:validated]
    		render obj as JSON
    }
...
def analysis={
  ...
  def basicStatistics = {
  		...
  		log.debug("Clearing grid in basicstatistics")
  		def result_instance_id1=params.result_instance_id1;
  		def result_instance_id2=params.result_instance_id2;

	
		def relation_concept = params.relation_concept;
		...
		log.debug(al.username)
		log.debug(al.event)
  		al.save()
  		log.debug("Result instance 1:"+result_instance_id1);
  		log.debug("Result instance 2:"+result_instance_id2);
  		def boolean s1=true;
		def boolean s2=true;
		...
		/* Trim results according to relationship between subset1 and subset2 */
		
		if (relation_concept != '') {
			def patnums=i2b2HelperService.getRelationsSubset(relation_concept, result_instance_id1, result_instance_id2);
		}
		...
		log.debug("Rendering age histograms")
		...
		if(s2 && l2.size()>0){dataset7.add(boxitem2, "Series 2", "Subset 2");}
			...
			pw.write("<td valign='top'><div style='position:relative;left:-30px;'></div></td>");
			pw.write("</td><td align='center'>");
	    ...
	    if (s1) {
		    ...
		    while (itr.hasNext()) {
		    	String s = itr.next()
				log.debug("examining: "+s+", count:"+raceResults1.get(s))
	    ...
	    if (s2) {
		    ...
		    while (itr.hasNext()) {
		    	String s = itr.next()
				log.debug("examining "+s+", count:"+raceResults2.get(s))
				if (raceResults2.get(s) != 0) {
		    		race2.put(s, raceResults2.get(s));
		    		log.debug("added to race2")
		    		if (! race1.containsKey(s)) {
		    			race1.put(s, 0)
			    		log.debug("also added to race2")
		    		}
		    	}
		    }
		    log.debug("race2: "+race2)
	    }
		...
		java.util.List<String> keys=i2b2HelperService.getConceptKeysInSubsets(result_instance_id1, result_instance_id2);
		pw.write("<hr>");
...
private void renderBoxAndWhiskerInfoTable(java.util.List<Number> values, PrintWriter pw)
{
	NumberFormat form;
...
//This should be in the helper service at the very least. Oi.
public void renderPatientCountInfoTable(String result_instance_id1, String result_instance_id2, PrintWriter pw)
{
	try{
...
private void renderBoxAndWhiskerInfoTableNew(java.util.List<Number> values,String trial, StringWriter pw)
{
	NumberFormat form;
	form=DecimalFormat.getInstance();
	form.setMaximumFractionDigits(2);
